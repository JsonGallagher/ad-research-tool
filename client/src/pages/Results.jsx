...
export default function Results() {
  ...
  // Clear analysis and landing page when changing ads
  useEffect(() => {
    setAdAnalysis(null)
  ...
  const fetchLandingPage = async () => {
    const url = selectedAd?.landing_url || selectedAd?.landingUrl
    if (!url) return
    ...
    try {
      ...
      if (result.error) {
        setLandingPageData({ error: result.error })
      } else {
        setLandingPageData(result.data)
      }
    } catch (error) {
      console.error('Landing page fetch error:', error)
      setLandingPageData({ error: error.message })
    } finally {
            ...
            <div className="bg-gradient-to-r from-emerald-50 to-teal-50 rounded-xl border border-emerald-200 p-5">
              ...
              <div className="space-y-2">
                {analysis.longevity.longest.map((item, i) => (
                  <div key={item.id || i} className="flex items-center justify-between bg-white rounded-lg p-3 shadow-sm">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <span className="font-medium text-gray-800">{item.advertiser || 'Unknown'}</span>
                        {item.adCount > 1 && (
                ...
                {(selectedAd.landing_url || selectedAd.landingUrl) && (
                  <>
                    <button
                      ...
                      disabled={isLoadingLandingPage}
                      className={`px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 ${
                        isLoadingLandingPage
                          ? 'bg-teal-200 cursor-not-allowed text-teal-600'
              ...
              {landingPageData && (
                <div className="mt-4 pt-4 border-t border-gray-200">
                  ...
                  {landingPageData.error ? (
                    <div className="text-red-600 text-sm p-3 bg-red-50 rounded-lg">
                      Could not load landing page: {landingPageData.error}
                    </div>
                  ) : (
                    <div className="space-y-4">
                      {/* Screenshot */}
                      {landingPageData.screenshot_path && (
                        <div className="bg-gray-50 rounded-lg p-2">
                          <img
                            src={`/screenshots/${landingPageData.screenshot_path}`}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                        {landingPageData.title && (
                          <div>
                            <span className="font-medium text-gray-500">Page Title:</span>
                        ...
                        {landingPageData.headline && (
                          <div>
                            <span className="font-medium text-gray-500">Main Headline:</span>
                            <p className="text-gray-800 font-medium">{landingPageData.headline}</p>
                          </div>
                        )}

                        {landingPageData.primary_cta && (
                          <div>
                            <span className="font-medium text-gray-500">Primary CTA:</span>
                            <span className="ml-2 px-2 py-1 bg-teal-100 text-teal-800 rounded text-xs font-medium">
                              {landingPageData.primary_cta}
                            </span>
                          </div>
                        )}
                        ...
                        {landingPageData.description && (
                          <div className="md:col-span-2">
                            ...
                            <p className="text-gray-600 mt-1">{landingPageData.description}</p>
                          </div>
                        )}
                      </div>

                      {/* Key Messaging */}
                      {landingPageData.key_messaging && landingPageData.key_messaging.length > 0 && (
                        <div>
                          <span className="text-sm font-medium text-gray-500">Key Messaging:</span>
                          <div className="mt-2 space-y-2">
                            {landingPageData.key_messaging.map((msg, i) => (
                              <p key={i} className="text-sm text-gray-600 bg-gray-50 p-2 rounded italic">
                                "{msg}"
                              </p>
                            ))}
                          </div>
                      ...
                      <div className="bg-teal-50 border border-teal-200 rounded-lg p-3">
                        <span className="text-sm font-medium text-teal-800">Ad vs Landing Page Comparison:</span>
                        <div className="mt-2 grid grid-cols-2 gap-4 text-xs">
                          <div>
                            <span className="font-medium text-gray-600">Ad CTA:</span>
                            <span className="ml-1 text-gray-800">{selectedAd.cta_text || selectedAd.ctaText || 'None'}</span>
                          </div>
                          <div>
                            <span className="font-medium text-gray-600">Page CTA:</span>
                            <span className="ml-1 text-gray-800">{landingPageData.primary_cta || 'None'}</span>
                          </div>
                        </div>
                      </div>
                    </div>
